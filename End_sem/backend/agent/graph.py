...
def route_demand(state: AgentState):
    """Routes to deep analysis if risk is High, otherwise skips to pattern detection."""
...
def route_evaluation(state: AgentState):
    ...
    - If confidence > 0.90 and no weakness feedback → END (plan accepted)
    - If confidence < 0.75 or feedback contains 'weak' and under 3 iters → loop back to reasoning
    ...
    if (confidence < 0.75 or "weak" in feedback.lower()) and iters < 1:
        return "reasoning"
...
def build_graph():
    """Constructs and compiles the LangGraph agentic workflow."""
